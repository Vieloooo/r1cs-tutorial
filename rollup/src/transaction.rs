...
impl TransactionVar {
    ...
    pub fn validate(
        ...
        let sig_verifies = self.verify_signature(&parameters.sig_params, &pre_sender_acc_info.public_key).unwrap(); 
        ...
        // post sender balance = pre blance (checked_sub) tx.amount
        
        // Question: does rust will automatically clone when passing a struct (derived clone) to a function which will occupy is ownership? 
        post_sender_acc_info.balance = pre_sender_acc_info.balance
        .checked_sub(&self.amount).unwrap();
        ...
        post_recipient_acc_info.balance = pre_recipient_acc_info.balance
        .checked_add(&self.amount).unwrap();
        ...
        // to check if sender exists, we just check if the pre_sender_acc node in the pre root tree
        let sender_exists = pre_sender_path.verify_membership(&parameters.leaf_crh_params, &parameters.two_to_one_crh_params, &pre_root, &pre_sender_acc_info.to_bytes_le().as_slice() ).unwrap(); 

        //check if the correct post sender info is the post merkle tree
        let sender_updated_correctly = post_sender_path.verify_membership(&parameters.leaf_crh_params, &parameters.two_to_one_crh_params, &post_root, &post_sender_acc_info.to_bytes_le().as_slice()).unwrap();
        ...
        let recipient_exists = pre_recipient_path.verify_membership(&parameters.leaf_crh_params, &parameters.two_to_one_crh_params, &pre_root, &pre_recipient_acc_info.to_bytes_le().as_slice() ).unwrap(); 

        // let recipient_updated_correctly = ???
        let recipient_updated_correctly = post_recipient_path.verify_membership(&parameters.leaf_crh_params, &parameters.two_to_one_crh_params, &post_root, &post_recipient_acc_info.to_bytes_le().as_slice()).unwrap();
